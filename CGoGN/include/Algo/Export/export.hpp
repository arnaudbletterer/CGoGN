...
template <typename PFP>
bool exportPLYVert(typename PFP::MAP& map, const VertexAttribute<typename PFP::VEC3, typename PFP::MAP>& position, const char* filename, bool binary)
{
	typedef typename PFP::MAP MAP;
...
bool exportPLYVertMaps(std::vector<typename PFP::MAP*>& maps,
					   const std::vector<std::vector<VertexAttribute<typename PFP::VEC3, typename PFP::MAP>>>& attributeHandlers,
					   const char* filename,
					   const bool binary)
{
	typedef typename PFP::MAP MAP;
	...
	{
		nbDarts += maps[i]->getNbDarts();
	}

	vertices.reserve(nbDarts/6) ;

	std::vector<unsigned int> nbVertices;
	nbVertices.reserve(maps.size());

	for(unsigned int i = 0; i < maps.size(); ++i)
	{
		MAP* map = maps[i];
		// Go over all vertices
		TraversorV<MAP> t(*map) ;
		for(Dart d = t.begin(); d != t.end(); d = t.next())
		{
			unsigned int vNum = map->template getEmbedding<VERTEX>(d) ;
			vertices.push_back(vNum) ;
		}
		nbVertices.push_back(vertices.size());
	}
	...
	out << "element vertex " << vertices.size() << std::endl ;
	// Position property
	for (unsigned int i = 0; i < attributeHandlers[0].size(); ++i)
	{
		VertexAttribute<typename PFP::VEC3, MAP> attrHandler = attributeHandlers[0][i];
		if (attrHandler.isValid() && (attrHandler.getOrbit() == VERTEX) )
		{
			...
			else if (attrHandler.name().compare("normal") == 0)	// Vertex normal property
			{
	...
	{
		// ascii vertices
		for(unsigned int i = 0; i < nbVertices[0]; ++i)
		{
			for (VertexAttribute<typename PFP::VEC3, MAP> attrHandler : attributeHandlers[0])
			{
				if (attrHandler.name().compare("position") || attrHandler.name().compare("normal") || attrHandler.name().compare("color"))
				{
		...
		for(unsigned int j = 1; j < maps.size(); ++j)
		{
			std::vector<VertexAttribute<typename PFP::VEC3, MAP>> attrHandlers = attributeHandlers[j];
			for(unsigned int i = nbVertices[j-1]; i < nbVertices[j]; ++i)
			{
				for (VertexAttribute<typename PFP::VEC3, MAP> attrHandler : attrHandlers)
				{
					if (attrHandler.name().compare("position") || attrHandler.name().compare("normal") || attrHandler.name().compare("color"))
					{
						out << attrHandler[vertices[i]];
	...
	else // binary
	{
		...
		for(unsigned int i = 0; i < nbVertices[0]; ++i)
		{
			for (VertexAttribute<typename PFP::VEC3, MAP> attrHandler : attributeHandlers[0])
			{
				if (attrHandler.name().compare("position") || attrHandler.name().compare("normal") || attrHandler.name().compare("color"))
				{
					Geom::Vec3f& v = attrHandler[vertices[i]] ;
					out.write((char*)(&(v[0])), sizeof(v)) ;
				}
			}
		}
